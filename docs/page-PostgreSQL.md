...
پس از نصب کتابخانه مربوطه در فایل ایجاد شده کد های ذیل را وارد اجرا کنید:
...
# واردکردن کتابخانه
import psycopg2
# اطلاعات اتصال به پستگرس.
DB_NAME = "test"             # نام جدول
DB_USER = "postgres"         # نام یوزر
DB_PASS = "12345"            # رمز عبور   
DB_HOST = "localhost"        # آدرس هاست
DB_PORT = "5432"             # پورت  
try:
...
# ایجاد جدول 
try:
   cur.execute("CREATE TABLE Persons (id serial PRIMARY KEY, names text);")
   print("Table is Created...")
except:
   print("I can't drop our test database!")
   
#----------------------------------------------------------------------------------    
# افزودن اطلاعات جدید به جدول    
try:
   cur.execute("INSERT INTO Persons (id, names) VALUES (1 , 'Ali')")
   print("Insert Name...")
except:
   print("Problem")    
# باز کردن نشانگر
...
# واکشی تمامی ردیف ها
rows = cur.fetchall()
# نمایش تمام ردیف های اطلاعاتی از جدول
for row in rows:
   print(row)
#----------------------------------------------------------------------------------

conn.commit()   # ارسال اطلاعات 
cur.close()     # بستن نشانگر پیش از بستن کانکشن
conn.close()    # بستن کانکشن       
```